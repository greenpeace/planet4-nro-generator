...
add_environment_variable.sh WP_DB_USERNAME "$(echo "${MYSQL_USERNAME}" | openssl base64 -e -A)" &
add_environment_variable.sh WP_DB_PASSWORD "$(echo "${MYSQL_PASSWORD}" | openssl base64 -e -A)" &
